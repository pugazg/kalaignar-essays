---
source_pdf: "TVA_BOK_0064140_ஆறுமாதக்_கடுங்காவல்.pdf"
scan_page: 181
printed_page: 178
publication: "aaru-maatha-kadungkaaval"
article: "sirai"
section: "sirai"
page_type: "body"
status: "verified"
language: "ta"
transcription_method: "direct visual comparison with user-supplied scans-150–224 source extract"
---

# அச்சு உரை

இங்கிலாந்து - அரசர்களாலும் பெருமை பெற்றிருக்கலாம். ஆனால் ஷேக்ஸ்பியர் ஏற்றிவைத்த புகழ்தீபம் என்றும் அணையாத சுடர் கொண்டது. பிரான்ஸ் என்றால் நெப்போலியனின் நினைவு வருவது போலவே- நாடக ஆசிரியர் மோலியேரின் நினைவும் வரத்தான் செய்கிறது. ஹாலந்து குடியரசான கதையைக் காணும் போது ஆரஞ்சு வில்லியம் எதிரே நிற்கிறான். ஹாலந்தின் ஓவிய நிபுணன் ராம் ராண்டும் நிழல்போல ஹாலந்தின் புகழைத் தொடருகிறான்.

இப்படி ஒவ்வொரு நாட்டிலும் இலக்கியமும் - கலையும் - ஓவியமும் - சிற்பமும் - ஆட்சியோடு போட்டிபோட்டு அந்நாட்டுக்குப் பசுமை குன்றாத புகழையும் பெற்றுத் தந்திருக்கின்றன. ஆட்சியோடு சேர்ந்து அந்நாடுகளின் பாலைவனத் தரையாகவும் ஆக்கியிருக்கின்றன. அதே நிலைமையை நாம் நமது திராவிடத்திலும் காண முடிகிறது.

பாயும் அருவி - பறக்கும் கிள்ளை - அழகு துள்ளும் மான் - ஆவேசம் அள்ளும் வேங்கை - இவைகளே ஓவியத்திலே சித்திரித்தார்கள்.

அந்நாளில் வீர பரம்பரை நாட்டை ஆண்டுவந்தது!

பிறகு - பாயும் அருவியிலே ஒரு தாமரை! தாமரை இதழிலே லட்சுமி! பறக்கும் கிள்ளையிலே மன்மதன்! அழகு துள்ளும் மான் - அந்த மாய மானைத் துரத்தும் ராமன் - அவனுக்கோர் புராணம்!

இந்நாளிலே பக்திமான்கள் நாட்டைப் பரிபாலித்தார்கள்! இது இரண்டாவது நிலை - வாழ்ந்து கெட்ட நிலை!

புறநானூறு கையிலே இருந்து ஒரு நாள் - புராண இதிகாசங்கள் இலக்கியமாயின மறுநாள்-எழுத்தெல்லாம் புதுமையெனும் இனிமை, மொட்டவிழ்கிறது இந்நாள்!

## அச்சு அமைப்பு

- visible printed folio — **178**.
- running header — `கருணாநிதி`.
- extract page **32** = physical scan **181**.
- source-visible forms including `போலவே-`, `ராம் ராண்டும்`, `போட்டிபோட்டு`, `கிள்ளை`, `ஆண்டுவந்தது`, and `மறுநாள்-எழுத்தெல்லாம்` were preserved without normalization.
- page ends with complete `மொட்டவிழ்கிறது இந்நாள்!`.

## அச்சு அல்லாத / physical-copy marks

- paper toning / scattered age specks are not publication text.
- no physical-copy mark obscures canonical wording.

## Historical glyph audit

- complete page inspected directly at enlarged/high-resolution source pixels.
- explicitly checked `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ`.
- representative sensitive clusters including `அணையாத`, `நினைவு`, `கிள்ளை`, `நிலை`, and `புறநானூறு` were checked by character identity.
- unresolved historical-glyph ambiguity — **0**.
- no global replacement or spelling modernization was used.
- cumulative historical-glyph correction-event count remains **11**.

## P2R visual-fidelity re-audit — Batch R35

- every printed word, punctuation mark, meaningful spacing, running header, visible folio and complete ending was rechecked directly from derivative extract page **32** / original physical scan **181**.
- `ராம் ராண்டும்`, `கிள்ளை`, `மறுநாள்-எழுத்தெல்லாம்`, and terminal `மொட்டவிழ்கிறது இந்நாள்!` were reconfirmed.
- paper toning / scattered age specks remain non-publication layer.
- mandatory historical-family verification was repeated; `அணையாத` (`ணை`), `கிள்ளை` (`ளை`), `நிலை` (`லை`) and the full 13-family set were checked by character identity.
- canonical correction — **none**.
- new historical-family correction events — **0**; cumulative count remains **20**.
- result — **PASS**.

## P2 நிலை

**verified** — scan 181 placeholder was replaced only after full direct source-pixel transcription and audit.
