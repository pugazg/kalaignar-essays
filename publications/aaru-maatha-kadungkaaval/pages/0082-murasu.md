---
source_pdf: "TVA_BOK_0064140_ஆறுமாதக்_கடுங்காவல்.pdf"
scan_page: 82
printed_page: 79
publication: "aaru-maatha-kadungkaaval"
article: "murasu"
section: "murasu"
page_type: "body"
status: "verified"
language: "ta"
transcription_method: "direct visual comparison with source scan"
---

# அச்சு உரை

நிலைகளுக்கிடையிலும் சுற்றிச் சுற்றி பிரசாரப் பணி புரியும் சுழல் விளக்கு. இளைத்த உடல் - வலுத்த உள்ளம் - அந்த இளைஞரேறு அறப்போர் முனையிலேயிருந்து ஆவன செய்தார். வீரர்களுக்கு ஆர்வமூட்டினார். இலட்சியக் கதைகளைப் பேசி பேசி அந்த இரவை ஓட்டினோம். அவர், நான், மணி, அம்பில், சத்தி, முத்து, கண்ணதாசன், ராமசுப்பையா, C. D. மூர்த்தி ஆகியோர்!

ஜூலை 15! விடிவெள்ளி முளைத்தது - வீழ்ந்த திராவிடத்திற்கும் விடுதலை உதயமாகப் போகிறது என்பதை அறிவிப்பதுபோல்!

சேவல், “கொக்கரக்கோ” எனக் கூவிற்று; அது எல்லோரும் “புறப்படுங்கோ!” என்பதுபோல் இருந்தது.

செங்கதிரோன் எழுந்தான் - வெங்கு நதி தனிற் அமிழ்ந்து வீரஞ் செய்கின்ற மூச்சோடு தமிழகமே எழுந்தது.

தமிழர் மீது தருக்கு மொழி வீசுவது வடவர்க்கு வாடிக்கை - ஆனால் வரலாறு சொல்கிறது; தருக்குமொழி பேசியோர் தலையொடியக் கல் சுமந்தனர் என்று! வரலாற்றை மறந்தனர் - வாடிக்கையை மறக்கவில்லை! அந்த வடநாட்டாருக்கு எச்சரிக்கை செய்வோம்.

ஏ! தமிழ் நிலமே! எழுந்திரு! என முழங்கியது கடல்!

கடலின் கொந்தளிப்பு உள்ளத்திலே! காலக் கதிரவனின் நிறம் கண்களிலே! சேவலின் மிடுக்கு நடையிலே!

தமிழர் பட்டாளம் எழுந்தது! எழுந்தது! எவரும் எமக்கு இணையில்லை - எனக் கூறி எழுந்தது!

## அச்சு அமைப்பு

- visible printed folio — **79**.
- running header — `ஆறுமாதக் கடுங்காவல்`.
- scan 81 terminal `நெருக்கடியான` directly continues with `நிலைகளுக்கிடையிலும்`.
- source-visible `இளைத்த உடல்`, `இளைஞரேறு`, `வெங்கு நதி தனிற்`, `தருக்கு மொழி`, and `தலையொடியக்` were preserved.
- page ends with complete `எனக் கூறி எழுந்தது!`.

## அச்சு அல்லாத / physical-copy marks

- source-visible stains / print wear are not transcribed as publication text.
- no physical obstruction hides printed wording.

## Historical glyph audit

- whole page inspected directly at enlarged source resolution.
- `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ` families were explicitly checked.
- `இளைத்த`, `இளைஞரேறு`, `இணையில்லை`, and affected clusters were decoded by historical character identity.
- unresolved historical-glyph ambiguity — **0**; cumulative correction count unchanged.

## P2R visual-fidelity re-audit — Batch R17

- scan 81 terminal `நெருக்கடியான` + scan 82 opening `நிலைகளுக்கிடையிலும்` was visually reconfirmed.
- every printed word, Latin initials, quotation mark, punctuation unit, meaningful spacing and complete page ending was compared directly with native source pixels.
- source-visible forms including `இளைத்த உடல்`, `இளைஞரேறு`, `பேசி பேசி`, `C. D. மூர்த்தி`, `வெங்கு நதி தனிற்`, `தருக்கு மொழி`, `தருக்குமொழி`, and `தலையொடியக்` all match the canonical record.
- mandatory historical-family verification was repeated; `இளைத்த` / `இளைஞரேறு` reconfirm `ளை`, `இணையில்லை` reconfirms `ணை`, and `கண்ணதாசன்` reconfirms `ணா`.
- unresolved historical-glyph ambiguity — **0**; cumulative historical-glyph correction-event count remains **11**.
- result — **PASS / no canonical text correction**.

## P2 நிலை

**verified** — scan 82 was fully compared with source pixels.
