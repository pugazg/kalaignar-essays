---
source_pdf: "TVA_BOK_0064140_ஆறுமாதக்_கடுங்காவல்.pdf"
scan_page: 95
printed_page: 92
publication: "aaru-maatha-kadungkaaval"
article: "murasu"
section: "murasu"
page_type: "body"
status: "verified"
language: "ta"
transcription_method: "direct visual comparison with source scan"
---

# அச்சு உரை

செய்யும் செட்டிமார் நாட்டிலே பிறந்தவன் நான்.” என்று பேச்சை ஆரம்பித்தார். குறைந்த நேரம் பேசினார். நமது கொள்கைகளையே பேசினார். அனைவரும் பூரித்தோம். அவர் நமது இயக்கத்துக்கு வர பொள்ளாச்சிதான் நுழைவு வாயிலாக இருந்தது. பிறகு அவருக்கு காரைக்குடியிலே திருமணம். நானும் அன்புச் சகோதரர் அரங்கண்ணலும் அங்கு சென்றிருந்தோம். திருமண வீட்டுக்குச் செல்லும் வழியில் ராமசுப்பையாவைக் கண்டோம். தனக்கும் தெரியாமல் ஒரு தோழர் இயக்கத்துக்கு வந்திருப்பதும் - அவர் மணத்திற்கு நாங்கள் செல்லுவதும் - அவருக்கு மிக வியப்பாகவேயிருந்தது. எல்லோரும் சென்றோம்.

குதிரையென்று சொல்லப்படும் ஒரு பிராணியில் மணமகன் அமர்த்தப்பட்டு - கால்கள் தரையைத் தொடமுடியாத நிலையில் - பொய்க்கால் குதிரை மகாராஜாபோல் வேஷம் போடப்பட்டு - எங்கள் நண்பர் ஊர்வலமாக வந்துகொண்டிருந்த அதியற்புதமான செட்டிநாட்டு திருமணங்களில் சிறப்புமிக்க வழக்கமான காட்சியைக் கண்டோம்.

சில இடங்களில் மாப்பிள்ளைக்கு சாமியார் வேஷம் போட்டு வாழ்வு நிலையாமையையும் - ‘கூசாமல் சந்நியாசங்கொள்’ என்ற உபதேசத்தையும் நிகழ்ச்சியாக்குவார்கள்.

செட்டிநாட்டிலோ; வாழ்வு ராஜபோகமானது - என்பதை வலியுறுத்துகிறார்கள் மாப்பிள்ளைமார்களுக்கு வேஷம் போட்டு!

மாலையில் திருமணப் பாராட்டுக் கூட்டம் - நாரண துரைக்கண்ணன் தலைமையில் நாங்கள் பேசினோம்.

## அச்சு அமைப்பு

- visible printed folio — **92**.
- running header — `கருணாநிதி`.
- scan 94 terminal `கும்பாபிஷேகம்` continues directly with this page’s `செய்யும் செட்டிமார் நாட்டிலே பிறந்தவன் நான்.”`
- source-visible `பொள்ளாச்சிதான்`, `வியப்பாகவேயிருந்தது`, `மகாராஜாபோல்`, `அதியற்புதமான`, `கூசாமல் சந்நியாசங்கொள்`, and `ராஜபோகமானது` were preserved.
- page ends with complete sentence `நாரண துரைக்கண்ணன் தலைமையில் நாங்கள் பேசினோம்.`
- scan 96 was not opened.

## அச்சு அல்லாத / physical-copy marks

- source-visible stains / print wear are not transcribed as publication text.
- no physical obstruction hides printed wording.

## Historical glyph audit

- whole page inspected directly at enlarged source resolution.
- `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ` families were explicitly checked.
- `அரங்கண்ணலும்`, `துரைக்கண்ணன்`, and affected older-type clusters were decoded by historical character identity.
- unresolved historical-glyph ambiguity — **0**; cumulative correction count unchanged.

## P2R visual-fidelity re-audit — Batch R19

- every printed word, numeral, quotation mark, punctuation mark, meaningful spacing, running header and complete page ending was rechecked directly from the attached source pixels.
- scan 94 terminal `கும்பாபிஷேகம்` → scan 95 opening `செய்யும் செட்டிமார் நாட்டிலே பிறந்தவன் நான்.”` continuation was reconfirmed exactly.
- source-visible forms including `பொள்ளாச்சிதான்`, `வியப்பாகவேயிருந்தது`, `மகாராஜாபோல்`, `அதியற்புதமான`, `கூசாமல் சந்நியாசங்கொள்`, and `ராஜபோகமானது` were preserved.
- physical stains / print wear remain non-publication layer; none obscures printed wording.
- mandatory historical-family verification was repeated; `நிலையாமையையும்`, `மாப்பிள்ளைமார்களுக்கு`, `மாலையில்`, and `தலைமையில்` reconfirm the `லை / ளை` families; the full 13-family set was checked by character identity.
- canonical correction — **none**.
- scan 96 was not opened or processed in this iteration.
- unresolved historical-glyph ambiguity — **0**; cumulative historical-glyph correction-event count remains **11**.
- result — **PASS**.

## P2 நிலை

**verified** — scan 95 was fully compared with source pixels.
