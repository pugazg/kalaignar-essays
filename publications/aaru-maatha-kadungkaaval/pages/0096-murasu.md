---
source_pdf: "TVA_BOK_0064140_ஆறுமாதக்_கடுங்காவல்.pdf"
scan_page: 96
printed_page: 93
publication: "aaru-maatha-kadungkaaval"
article: "murasu"
section: "murasu"
page_type: "body"
status: "verified"
language: "ta"
transcription_method: "direct visual comparison with source scan"
---

# அச்சு உரை

செட்டிநாட்டு நண்பரும் நாங்களும் - பிறகு - மிகவும் நெருங்கிவிட்டோம். கழகக் காரியங்கள் - சொந்த அலுவல்கள் - எல்லாவற்றிலும் அவரும் துணைபுரிபவரானார்.

அவர் மிகவும் நல்லவர். திருநீறு பூசியிருந்தாரே - அப்போதுங்கூட! அதனால் தான் அவர் அவ்வளவு சீக்கிரம் நம் பாசறையை உணர முடிந்தது - உழைப்பார் - உணர்ச்சியோடு செயலாற்றுவார் - ஒரே குறை - எனக்கும் அவருக்கும் அடிக்கடி தகராறு உண்டாகும் குறை - உணவு விஷயத்தில் அவருக்கு அவரே நிகர்! சிறைச்சாலைக்குப்போனால் என்னய்யா செய்வீர்; என்று கோபமாகக் கேட்டேன். “அதற்குத்தான் தயார்படுத்துகிறேன்” என்பார்!

“ஒட்டகம்போல தயார்படுத்த முடியாது தோழரே! ஒத்திகை பார்த்துக்கொள்ளவேண்டும் பட்டினி கிடந்து!” என்பேன்! ஆத்திரம் தாங்கமுடியாமல் இலையைத் தூக்கி விட்டெறிவார் - எச்சில் தொட்டியில் - எல்லாம் சுத்தமாக சாப்பிட்டான பிறகு!

உணவு விஷயத்தில் நல்ல பாடங்கற்பிக்க சமயம் பார்த்திருந்தேன். கிடைத்துவிட்டது சந்தர்ப்பம். ஆனால் அது நழுவிவிட்டது. (இருந்தாலும் மீண்டும் கிடைக்கும்)

கல்லக்குடி போராட்டத்தில் மூன்றும் படை வரிசைக்கு தளபதியாகச் சென்று - போலீசாரின் முரட்டுத்தனமான தாக்குதலுக்கு ஆளான கண்ணதாசனைப் பற்றித்தான் இவ்வளவு தூரம் கூறுகிறேன் என்பது உங்களுக்குத் தெரியாமலா இருக்கும். ஜூலை 14ல் லால்குடி வந்தார்! மேடையில் பேசினார். ‘குருவுக்கு வரும் துன்பம் சிஷ்யனை

## அச்சு அமைப்பு

- visible printed folio — **93**.
- running header — `ஆறுமாதக் கடுங்காவல்`.
- enlarged source-pixel recheck corrected `உண்டாகும் குறை`, `கேட்டேன்`, and `தயார்படுத்துகிறேன்`; these readings are now source-controlled.
- source-visible unusual forms including `சாப்பிட்டான பிறகு!`, `பாடங்கற்பிக்க`, and `மூன்றும் படை வரிசைக்கு` were preserved without normalization.
- page ends inside an open quotation at `சிஷ்யனை`; scan 97 continues directly with `யும் தாக்கட்டும்’`.

## அச்சு அல்லாத / physical-copy marks

- source-visible stains / paper aging are not transcribed as publication text.
- no physical-copy mark obscures body wording.

## Historical glyph audit

- whole page inspected directly at enlarged source resolution.
- `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ` families were explicitly checked.
- affected older-type clusters were decoded by character identity rather than modern visual resemblance.
- unresolved historical-glyph ambiguity — **0**; cumulative correction count unchanged.

## P2 நிலை

**verified** — scan 96 was fully compared with source pixels, including post-batch enlarged-pixel recheck.
