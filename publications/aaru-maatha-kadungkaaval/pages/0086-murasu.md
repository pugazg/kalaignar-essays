---
source_pdf: "TVA_BOK_0064140_ஆறுமாதக்_கடுங்காவல்.pdf"
scan_page: 86
printed_page: 83
publication: "aaru-maatha-kadungkaaval"
article: "murasu"
section: "murasu"
page_type: "body"
status: "verified"
language: "ta"
transcription_method: "direct visual comparison with source scan"
---

# அச்சு உரை

குறள் ஒலிக்கும் மாதா - வளையாபதியுடைய குண்டல கேசி - வஞ்சகர் பூட்டிய விலங்கால் வாட்டமுற்றுக் கிடந்தாள்!

அவள் வாட்டம் போக்க வந்துவிட்டோம் நாங்கள்! எங்கள் செல்வத் திருநாட்டை சித்திரவதை செய்யும் சிலுவையை போலீசார் புடைகுழ்ந்து காத்தனர்.

நாங்கள் விலக்கவந்த விலங்கிற்கு துப்பாக்கி வேலி போட்டனர்.

துடித்தெழும் மாதாவை - தூரத்திலேயிருந்து பார்! தொடாதே! அவள் தொல்லையுறுவதை ரசி! என்று அரசாங்கம்!

கோழைகள் கேட்பர் - மோழைகள் பின்னடைவர் - “நாங்கள் வாழைக்குக் கன்றல்ல - ஆளுக்கு விழுதுகள்” - அன்னை திராவிடத்திற்கு மக்கள் - இப்படிப் பதில் தந்தோம்! ஓடினோம்; அன்னையிடம்! முள் கிரீடத்தைக் கழற்றி எறிந்தோம் - ரத்தம் கசியும் நெற்றியிலே மருந்து வைத்தோம்.

“டால்மியாபுரம்” என்ற பெயரை மறைத்தோம். “கல்லக்குடி” என்ற பெயரை அந்தப் பலகை மீது ஒட்டினோம்.

வடநாட்டான் வசமுள்ள புகைவண்டி வந்து நின்றது. “அதிகாரபூர்வமாகப் பெயரை மாற்று! அதன் பிறகு ரயில் செல்லட்டும்” என்று முழங்கினோம். நாங்கள் இருபத்தைந்துபேர் - இலட்சிய பேரிகை ஒலித்தோம். புகைவண்டி புறப்பட ஆயத்தமானது.

## அச்சு அமைப்பு

- visible printed folio — **83**.
- running header — `ஆறுமாதக் கடுங்காவல்`.
- scan 85 terminal `மாணிக்க` continues directly with this page’s `குறள் ஒலிக்கும் மாதா...`.
- source-visible unusual forms including `புடைகுழ்ந்து`, `மோழைகள்`, `வாழைக்குக் கன்றல்ல - ஆளுக்கு விழுதுகள்`, and `இருபத்தைந்துபேர்` were preserved without normalization.
- page ends with complete sentence `புகைவண்டி புறப்பட ஆயத்தமானது.`

## அச்சு அல்லாத / physical-copy marks

- source-visible stains / age marks are not transcribed as publication text.
- no physical-copy mark obscures body wording.

## Historical glyph audit

- whole page inspected directly at enlarged source resolution.
- `ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ` families were explicitly checked.
- affected older-type clusters were decoded by character identity rather than modern visual resemblance.
- unresolved historical-glyph ambiguity — **0**; cumulative correction count unchanged.

## P2R visual-fidelity re-audit — Batch R18

- every printed word, numeral, punctuation mark, meaningful spacing, running header and page-boundary continuation was rechecked directly from the attached source pixels.
- scan-85 terminal `மாணிக்க` → scan-86 opening `குறள் ஒலிக்கும் மாதா...` continuation was confirmed without rewriting either physical page record.
- source-visible forms including `புடைகுழ்ந்து`, `மோழைகள்`, `வாழைக்குக் கன்றல்ல - ஆளுக்கு விழுதுகள்`, and `இருபத்தைந்துபேர்` were reconfirmed and preserved.
- physical stains / age marks are non-publication layer; none obscures printed wording.
- mandatory historical-family verification was repeated; `வளையாபதியுடைய` reconfirms `ளை` and `அன்னையிடம்` reconfirms `னை`; the full 13-family set was checked by character identity.
- canonical correction — **none**.
- unresolved historical-glyph ambiguity — **0**; cumulative historical-glyph correction-event count remains **11**.
- result — **PASS**.

## P2 நிலை

**verified** — scan 86 was fully compared with source pixels.
