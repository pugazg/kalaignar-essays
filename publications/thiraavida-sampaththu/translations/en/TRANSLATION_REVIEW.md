...
The Tamil archival layer is **COMPLETE / STRICT-REVIEWED / FROZEN** after P5 **16 / 16 PASS**.
...
1. English translates the frozen Tamil assemblies, not OCR, the pre-P5 transcription, alternate scan readings or memory.
2. Article 1 retains source-page trace comments for scans **5, 6, 13, 14, 15, 16**.
3. Article 2 must retain source-page trace comments for scans **12, 3**.
4. Article 1 scan-15/16 `SOURCE DAMAGE` gaps remain explicit; hidden wording is never reconstructed.
5. Frozen scan-3 `அயம்` remains damaged/ambiguous and cannot be silently repaired.
6. P5 lexical-witness conflicts remain frozen Tamil choices for translation.
7. Publisher foreword `மன்றத்தில்`, publisher note `முக்கிய குறிப்பு` and scan-4 advertisements remain outside both Kalaignar article translations.
...
- hidden torn wording reconstructed after T2: **0**;
