---
scan_page: 9
reading_order: 9
printed_page: 8
publication: "viduthalai-kilarcci"
article: "viduthalai-kilarcci"
section: "viduthalai-kilarcci"
page_type: "body"
status: "verified"
language: "ta"
source_filename: "TVA_BOK_0064066_விடுதலைக்கிளர்ச்சி.pdf"
transcription_method: "direct word-by-word visual comparison with source scan; user-supplied viduthalai_kilarchi.md used only as a non-authoritative baseline"
---

# அச்சு உரை

தும் துடித்துப் பார்க்கிறது விடுதலைக்காக! தவளையின் வாயிலே சிக்கிய பூச்சி, பாம்பின் பல்லிலே அகப்பட்ட தவளை, கருடன் நகத்திலே மாட்டிய பாம்பு, ஓநாயின் பார்வையில் பட்ட கருடன், கணை விடுவோன் கண்ணிலே வீழ்ந்த ஓநாய், இவை ஒவ்வொன்றுமே விடுதலைக்காகத்தான் போராட்டம் நடத்துகின்றன; இறந்துவிடுவோம் என்பது உறுதியாகத் தெரிந்துங்கூட!

நடைமுறைகள் மட்டுமல்ல; கடவுளரின் நிலைமை கூட இதேதான். கடவுளர் சம்மந்தப்பட்ட ஓவியங்களைக் காணும்போது விடுதலைக்கிளர்ச்சியின் சாயல் அங்கே படர்ந்திருப்பது நன்றாகத் தெரிகிறது. கணபதியின் பருத்த தொந்தியையும், பெருத்த தலையையும் தாங்கமுடியாமல் பெருச்சாளி கஷ்டப்படுகிறது. முருகனின் திருவுருவத்தை சித்தரிக்கும் ஓவியத்திலே மயிலின் காலில் தொங்கும் பாம்பு விடுதலைக்காக கிளர்ச்சி நடத்துகிறது. மயிலோ வள்ளியையும் தெய்வானையையும் சேர்த்து சுமக்க முடியாமல் விடுதலைக் கிளர்ச்சி நடத்துகிறது. வள்ளி-தெய்வானைகளோ முருகனின் பன்னிரு கைகளின் பலமான அணைப்பை சகிக்க முடியாமல் விடுதலைக் கிளர்ச்சி நடத்துகிறார்கள். முருகனோ; பாவம் பக்தர்களிடமிருந்து விடுதலைபெற கிளர்ச்சி நடத்துகிறார்.

இப்படி-அங்கிங்கெனாதபடி எங்கெங்கும் விடுதலைக் கிளர்ச்சியின் நிழல் தெரிந்து கொண்டிருக்கிறது.

## அச்சு அமைப்பு

- running head **விடுதலைக் கிளர்ச்சி** visible at top left.
- printed folio **8** visible at top right.
- opening **தும்** directly continues scan 8 **தெரிந்**.

## அச்சு அல்லாத / physical-copy marks

- ageing / light show-through only.

## Historical glyph audit

- full page checked word-by-word.
- historical `றா` identity in **நன்றாகத்** was decoded from the old-form glyph rather than its modern visual resemblance.
- repeated **நடத்துகிறார்கள் / நடத்துகிறார்** forms were checked directly.
- unresolved historical-glyph ambiguity — **0**.

## Baseline-to-source fidelity findings

- baseline line-wrap fragments were rejoined to **கணபதியின் / தெய்வானையையும் / விடுதலைபெற** where the source word itself is continuous.
- historical-looking baseline form around **நன்றாகத்** resolved from source pixels.
- no external mythological wording or explanatory gloss was introduced.

## P5 strict visual fidelity findings

- **`இப்படி அங்கிங்கெனாதபடி`** → source **`இப்படி-அங்கிங்கெனாதபடி`**.
- all other visible words, punctuation, folio and continuation **`தெரிந் / தும்`** rechecked — **PASS**.
- unresolved P5 readings — **0**.

## P2 நிலை

**verified** — scan 9 canonical text corrected under P5; guessed readings **0**; unresolved readings **0**.

<!-- மூல ஸ்கேன் பக்கம்: 9; அச்சுப் பக்கம்: 8 -->
