# மூலப் பதிவு — இன முழக்கம்

## Source identity

- Source filename: `TVA_BOK_0063958_இன_முழக்கம்.pdf`
- Source PDF committed to repository: **No**
- SHA-256: `f57e4070051d7bc77ab78d5d393dbefbe47791efcc3203c594c5f3949ef0dfbf`
- File size: **68,109,788 bytes**
- Physical scans: **50**
- Language/script: Tamil
- Publication type: **multi-unit pamphlet / essay-and-poetry collection**

## Printed identity witnesses

- cover title: `இன முழக்கம்`
- title-page title: `இன முழக்கம்`
- cover author witness: scan visibly carries abbreviated `மு. கருணாநிதி`; user lexical baseline retains `கலைஞர் கருணாநிதி`
- title-page author: `கலைஞர் கருணாநிதி`
- seller: `பாரி நிலையம்`, `59, பிராட்வே, சென்னை.`
- edition: `முதற் பதிப்பு: செப்டம்பர் 1951`
- price: `விலை அணா 0-8-0`
- publisher: `முன்னேற்றப் பண்ணை`, `சென்னை.`
- printer: `கே. ஜி. பிரஸ், சென்னை—1.`

## User-established lexical rule

> **Keep the words. Correct only structural order, punctuation, spacing, paragraphing, headings, misplaced words and analogous layout issues; remove unwanted non-source/OCR words.**

Operational authority:

1. user-supplied transcription = lexical baseline;
2. scan = structure, punctuation, quotation scope, headings, paragraph/page placement, poetry lineation and physical-copy evidence;
3. scan/baseline lexical or numeric disagreements are documented, not silently substituted;
4. stamps, handwriting, show-through, damage and marginal marks remain outside printed body text.

## Contents witness

1. `இன முழக்கம்` — `4`
2. `சொர்க்க லோகத்தில்` — `13`
3. `முரசறைவாய்` — `24`
4. `பழிக்குப் பழி` — `29`
5. `ஆரியம் பேசுகிறது` — `37`
6. `கவிதைகள்` — `40`

Contents-page numbers remain independent source witnesses from directly visible body numerals.

## Publication units

- scans 1–5 — front matter / `பதிப்புரை`
- scans 6–13 — `இன முழக்கம்`
- scans 14–24 — `சொர்க்க லோகத்தில்`
- scans 25–29 — `முரசறைவாய்`
- scans 30–37 — `பழிக்குப் பழி`
- scans 38–39 — `ஆரியம் பேசுகிறது`
- scan 40 — `கவிதைகளைப் பற்றி` + `மதிப்புரை`
- scans 41–49 — `கவிதைகள்`
- scan 50 — catalogue / advertisement

## Permanent non-regression findings

- scan 10: `கர்வத்தால் கனத்துப்போன கனக விசயர் தலைகளில்...`;
- scan 21→22: `நாயன்மார்கள்` begins on scan 22 and joins `ஸ்தம்பித்து விட்டார்கள்.`;
- scan 22 marginal printed `2` is non-body evidence, not OCR noise;
- scan 24 promotion remains outside `சொர்க்க லோகத்தில்`;
- scan 36→37 boundary: `இந்த மதத்தைப் பழிக்குப்` / `பழி வாங்க வேண்டாமா?`;
- scan 37 promotion remains outside `பழிக்குப் பழி`;
- scan 40 remains outside `கவிதைகள்`;
- scan 41 heading is `ஏற்பரோ!`;
- scan 49 heading is `வருணமா? மரணமா?`;
- scan 50 catalogue remains outside all six body assemblies.

## Gate status

- P0: **COMPLETE**
- P1: **COMPLETE**
- P2 canonical page records: **50 / 50 COMPLETE**
- P3 assemblies: **6 / 6 COMPLETE**
- P4 source/completeness audit: **PASS**
- P5 strict visual fidelity: **50 / 50 PASS**
- P5 assembly recheck: **6 / 6 PASS**
- Tamil archival layer: **COMPLETE / STRICT-REVIEWED / FROZEN**
- E0 English translation planning/setup: **COMPLETE / PASS**
- T0 frozen-source registration: **6 / 6 PASS**
- T1 close drafts: **2 / 6 PASS**
- T2 fidelity reviews: **2 / 6 PASS**
- T3 voice reviews: **1 / 6 PASS**
- T4 audits: **1 / 6 PASS**
- T5 verified: **1 / 6 PASS**
- E6/E7: **NOT STARTED**
- blockers: **0**

Canonical P5 review: [`../VISUAL_TEXT_FIDELITY_REVIEW.md`](../VISUAL_TEXT_FIDELITY_REVIEW.md).  
English translation plan: [`../TRANSLATION_PLAN.md`](../TRANSLATION_PLAN.md).

## Frozen Tamil authorities

1. `articles/01-ina-muzhakkam.md` — `df5907d3d6fa4981a2377fe621bf6f2d7ecc1010`
2. `articles/02-sorgga-logaththil.md` — `8fea1497ed2dcbdb395418590c1b9875e6066b8c`
3. `articles/03-murasaraivai.md` — `27ab358ac7cd06bc3c072e4bca425a1269f2c6b8`
4. `articles/04-pazhikku-pazhi.md` — `e140df7f6234e8362b9139793706533fec0c62d8`
5. `articles/05-aariyam-pesugirathu.md` — `e5bf851cd8384dfd9a84c1607c5f22cc5fa59675`
6. `articles/06-kavithaigal.md` — `6f0f6fc7ac0ed3132172d92bc1fa0378528c790c`

## English Article 1 T5 provenance

- verified English title: **The Clarion Call of the Race**
- English file: `../translations/en/01-ina-muzhakkam.md`
- source Tamil blob: `df5907d3d6fa4981a2377fe621bf6f2d7ecc1010`
- T5 verified English blob: `01d0d16c4f52cb134eba9fd35c06ad8376e256cb`
- status: `verified`
- source-page comments: **8 / 8 preserved in order**
- blockers: **0**

## English Article 2 T2 provenance

- working English title: **In the Heavenly Realm**
- English file: `../translations/en/02-sorgga-logaththil.md`
- source Tamil blob: `8fea1497ed2dcbdb395418590c1b9875e6066b8c`
- T1 English blob: `b02cc0e167e1f69aa324d5b761878f6be7134b5f`
- T2 English blob: `b9ed854519806bbe65dc5b006183fb4675cb2c3a`
- status: `fidelity-reviewed`
- source-page comments: **11 / 11 preserved in order**
- T2 fidelity body corrections: **3**
- blockers: **0**

T2 re-fetched the frozen Tamil and T1 English and compared every paragraph/clause. It corrected `சுழல் வண்டுகளென` from **whirling beetles** to **whirling bees**; clarified the Kotpuli Nayanar killing sentence while preserving the scan-18→19 boundary; and translated the mother's contextual `தம்பி` address as **son** rather than literal **little brother**. It retained the source-distinct Article-2 names/labels, all 11 page comments, page-boundary traces and the source-irregular incomplete `மாதாவை மகா மனைவியாக்குவது மல்லவா` treatment.

## English translation exclusions / constraints

- scans 24 and 37 promotional blocks are not body translation inputs;
- scan 40 `கவிதைகளைப் பற்றி` + `மதிப்புரை` remains outside `கவிதைகள்` translation;
- scan 50 catalogue remains outside all six English bodies;
- known P5 lexical-witness differences are not licences to translate from the scan alternative;
- Article 1 verified English decisions must not be reopened without a genuine source-supported defect;
- `கவிதைகள்` English must preserve frozen headings and lineation;
- source-page comments are required for bilingual traceability.

Exact next activity: **Article 2 `சொர்க்க லோகத்தில்` — T3 Kalaignar voice review only; stop before T4.**