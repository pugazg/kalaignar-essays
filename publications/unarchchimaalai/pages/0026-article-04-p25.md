---
scan_page: 26
printed_page: 25
publication: "unarchchimaalai"
article: "04-iravanan-nam-pattan"
section: "article"
page_type: "article-continuation"
status: "verified"
language: "ta"
source_filename: "TVA_BOK_0063821_உணர்ச்சிமாலை.pdf"
transcription_method: "Gemini lexical baseline retained; punctuation, paragraphs and verse lineation checked directly against source scan"
---

# அச்சு உரை

> மிடைமுலைக் குவடேறி மேகலைத் தடங்  
> கடலிடைப் புகுந்தகட் குலுழியாரரோ!”

பொழிப்புரையாகக் கூறுவதே போதுமென் எண்ணுகிறேன்; அண்ணா அவர்கள் கம்பரச புத்தகத்தில் அதிக விளக்கம் இருப்பதால் அதைப் படித்துக் கொள்ளட்டும் என்ற அளவில்!

இராமன் பிரிவால் மங்கையர் கண்ணில் வடியும் நீர், ஆறாகப் பெருகி குங்குமத்தையும் மையையும் கரைத்துக்கொண்டு, மணிமாலைகளைக் கடந்து, ஸ்தனங்கள் என்ற மலைகளில் ஏறி மேகலாபரணம் அணிந்திருக்கிற “மறைவிடம்” எனப்படும் கடலில் விழுகிறது.

“எழுதவே வெட்கமாயிருக்கிறது தோழரே! இந்தக் காமக்கலை கடவுளின் பேரால் பாடப்பட வேண்டுமா என்று கேட்பது உமக்கு வெறிச்செயலாகப் படுகிறதா?” எனக் கலாரசிகரான என் நண்பரைக் கேட்கிறேன்.

ஒருவேளை “அவரவர் நிலைக்கு ஏற்ப அமைந்ததோர் அழகுக் கோலம் தாங்குவது கலை.” என்ற தத்துவத்தைக் கடைப்பிடிப்பவராயிருக்கலாம் நண்பர்!

பாரதிதாசனையும் -கம்பரையும் ஒரே கோட்டில் நிறுத்திப் பார்க்கும் தோழர், பாரதிதாசனின் கவிதையில் இந்தக் காமக் களஞ்சியத்தைக் காணமுடியுமா என்று அறை கூவியழைக்கிறேன். வர்ணனைக்கும் அளவிருக்க வேண்டாமா? அதுவும் பரந்தாமன் சரித்திரத்திலா இந்தப் பள்ளியறைச் சிறப்பு?...

## அச்சு அல்லாத / physical-copy marks

- Aged paper, small specks and faint reverse-side show-through are visible.

## Audit note

- Visible printed page numeral `25` is recorded.
- The verse quotation begun on scan 25 is closed on this scan, preserving the page boundary.
- Gemini word tokens are retained; punctuation-only artifacts such as the stray colon in `பொழிப்புரையாகக் :` and doubled `!.` were corrected from the scan.
- No lexical normalisation was performed.

<!-- மூல ஸ்கேன் பக்கம்: 26; அச்சுப் பக்கம்: 25 -->