...
- Gemini word tokens are retained; OCR line-wrap spaces in `நிஜு லினா`, `இடோ கியோ` and similar forms were joined as spacing corrections only.
- OCR artefact `ID` from the supplied Gemini stream has no printed witness on the scan and is excluded as non-source text, analogous to intrusive OCR page-number artefacts.
- Paragraph boundaries and quotation punctuation were checked directly against scan 31.
